@extends('layouts.app')
@php
    // ReelShort detail API: {success, bookId, title, cover, description, totalEpisodes,
    //   chapters:[{index, chapterId, title, isLocked, serialNumber}]}
    // Also from foryou list: {book_id, book_title, book_pic, special_desc, chapter_count,
    //   theme:[], tag_list:[{tag_id, tag_name}]}
    $d = $detail;

    $title  = data_get($d, 'title') ?? data_get($d, 'book_title') ?? data_get($d, 'data.title') ?? 'Drama';
    $cover  = data_get($d, 'cover') ?? data_get($d, 'book_pic') ?? data_get($d, 'data.cover') ?? '';
    $desc   = data_get($d, 'description') ?? data_get($d, 'special_desc') ?? data_get($d, 'data.description') ?? '';
    $eps    = data_get($d, 'totalEpisodes') ?? data_get($d, 'chapter_count') ?? data_get($d, 'data.totalEpisodes') ?? '';

    // Chapters from detail API
    $chapters = data_get($d, 'chapters', []);
    $chapters = is_array($chapters) ? $chapters : [];

    // Tags: from tag_list [{tag_id, tag_name}] or theme []
    $tags = data_get($d, 'tag_list') ?? data_get($d, 'theme') ?? data_get($d, 'data.tag_list') ?? [];
    if (!is_array($tags)) $tags = [];
    $tagNames = array_filter(array_map(fn($t) => is_array($t) ? ($t['tag_name'] ?? $t['name'] ?? '') : $t, $tags));

    // Status
    $status = data_get($d, 'status') ?? data_get($d, 'book_status') ?? '';
...
{{-- Episode List from chapters array --}}
@if(count($chapters) > 0)
<div class="detail-episodes-section">
    <h3 class="episodes-heading">Daftar Episode <span class="ep-count-badge">{{ count($chapters) }}</span></h3>
    <div class="episode-grid">
        @foreach($chapters as $ch)
            @php $epNo = data_get($ch, 'index') ?? data_get($ch, 'serialNumber') ?? ($loop->index + 1); @endphp
            <a href="{{ route('reelshort.watch', ['id' => $bookId, 'ep' => $epNo]) }}" class="ep-btn {{ data_get($ch, 'isLocked') ? 'ep-locked' : '' }}">{{ $epNo }}</a>
        @endforeach
    </div>
</div>
@elseif($eps && (int)$eps > 0)
{{-- Fallback: generate episode numbers if chapters not available --}}
<div class="detail-episodes-section">
